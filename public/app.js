...
function productName(product) { return product.name || "Untitled frame"; }
...
function productCard(product) {
  ...
  return `<article class="card product-card">
    <a class="product-image" href="#product/${encodeURIComponent(product.id)}" style="${productImageStyle(product)}" aria-label="View ${esc(productName(product))}"></a>
    <div class="product-content"><span class="pill pill--soft">${esc(categoryTitle(product.category))}</span><h3>${esc(productName(product))}</h3>${meta ? `<p>${esc(meta)}</p>` : ""}<div class="product-meta"><span class="pill pill--warm">${esc(product.availability || "Ask in store")}</span></div><button class="button button--ghost button--small" data-action="inquire" data-product="${esc(product.id)}">Ask about this frame</button></div>
  </article>`;
}
function categoryTitle(id) { return CATEGORIES.find(c => c.id === id)?.title || "Uncategorized"; }
...
function home() {
  ...
  return layout(`
    <section class="hero"><img class="hero-image" src="${c.heroImage}" alt="Customer trying on glasses with guidance inside a modern optical store" fetchpriority="high"><div class="container"><div class="hero-copy"><p class="eyebrow">Local eyewear · ${esc(c.locationLabel)}</p><h1>${esc(c.heroTitle).replace("\n", "<br>")}</h1><p>${esc(c.heroDescription)}</p><div class="hero-actions"><a class="button button--light" href="#eyewear">Explore eyewear <span>↗</span></a><a class="button button--amber" href="#eye-test">Book an eye test</a><a class="button button--ghost" style="color:#fff;border-color:#bfc6bc" href="#contact">Visit our store</a></div></div></div></section>
    <section class="quick-actions" aria-label="Quick contact"><a href="${phoneHref()}"><span class="symbol">◔</span><span>Call us</span></a><a href="${waHref(defaultMessage())}" target="_blank" rel="noopener"><span class="symbol">◌</span><span>WhatsApp</span></a><a href="${esc(c.mapUrl)}" target="_blank" rel="noopener"><span class="symbol">⌖</span><span>Directions</span></a></section>
...
function adminContent(data, approved) { if (adminTab === "overview") return `<div class="metric-grid"><div class="card metric"><strong>${data.inquiries.filter(x=>x.status==="new").length}</strong><span>New inquiries</span></div><div class="card metric"><strong>${data.appointments.filter(x=>x.status==="pending").length}</strong><span>Pending appointments</span></div><div class="card metric"><strong>${data.reviews.filter(x=>x.status==="pending").length}</strong><span>Reviews to review</span></div><div class="card metric"><strong>${approved}</strong><span>Approved reviews</span></div></div><section class="card admin-section"><h2>What needs attention</h2><p class="admin-empty">${data.inquiries.filter(x=>x.status==="new").length || data.appointments.filter(x=>x.status==="pending").length || data.reviews.filter(x=>x.status==="pending").length ? "New customer submissions are ready in their respective tabs." : "Nothing is waiting right now. New customer submissions will appear here."}</p></section>`;
  ...
  if (adminTab === "products") {
    ...
    return `<section class="card admin-section"><h2>Showcase products</h2><p class="admin-empty">Products are display and inquiry only. They never become products for online purchase.</p><div class="product-admin">${data.products.map(product => `<div class="product-admin-row"><div style="display:flex;gap:.75rem;align-items:center">${product.image ? `<img src="${esc(product.image)}" alt="" style="width:56px;height:56px;object-fit:cover;border-radius:.5rem;flex-shrink:0">` : `<div style="width:56px;height:56px;border-radius:.5rem;background:#e7e2d8;flex-shrink:0"></div>`}<div><strong>${esc(product.name || "Untitled frame")}</strong><p>${esc(categoryTitle(product.category))}${product.shape ? ` · ${esc(product.shape)}` : ""} · ${esc(product.availability || "Ask in store")}</p></div></div><div style="display:flex;gap:.4rem;flex-wrap:wrap"><button class="button button--ghost button--small" data-action="edit-product" data-id="${product.id}">Edit</button><button class="button button--ghost button--small" data-action="toggle-featured" data-id="${product.id}">${product.featured ? "Remove feature" : "Feature"}</button><button class="button button--danger button--small" data-action="delete-product" data-id="${product.id}">Delete</button></div></div>`).join("")}</div></section><section class="card form-card" style="margin-top:1rem"><h2>${editing ? `Edit “${esc(editing.name || "Untitled frame")}”` : "Add a showcase frame"}</h2><p class="admin-empty">Every field below is optional — leave anything blank you don't want to fill in yet.</p><form data-form="product" class="form-grid"><div class="form-grid form-grid--two"><label>Frame name<input name="name" value="${editing ? esc(editing.name) : ""}" placeholder="e.g. Horizon Aviator"></label><label>Category <span>(optional)</span><select name="category"><option value="">Uncategorized</option>${CATEGORIES.map(c=>`<option value="${c.id}" ${editing?.category===c.id?"selected":""}>${c.title}</option>`).join("")}</select></label></div><div class="form-grid form-grid--two"><label>Shape <span>(optional)</span><input name="shape" value="${editing ? esc(editing.shape) : ""}"></label><label>Material <span>(optional)</span><input name="material" value="${editing ? esc(editing.material) : ""}"></label></div><div class="form-grid form-grid--two"><label>Colour <span>(optional)</span><input name="color" value="${editing ? esc(editing.color) : ""}"></label><label>Age group <span>(optional)</span><input name="ageGroup" value="${editing ? esc(editing.ageGroup) : ""}"></label></div><label>Availability<input name="availability" value="${editing ? esc(editing.availability) : "Ask in store"}"></label><label>Description <span>(optional)</span><textarea name="description">${editing ? esc(editing.description) : ""}</textarea></label><label>Photo ${editing?.image ? "<span>(uploading a new one replaces the current photo)</span>" : "<span>(optional — JPEG, PNG, or WEBP, under 4MB)</span>"}<input type="file" name="photo" accept="image/png,image/jpeg,image/webp"></label><div style="display:flex;gap:.6rem"><button class="button button--primary" type="submit">${editing ? "Save changes" : "Add showcase frame"}</button>${editing ? `<button class="button button--ghost" type="button" data-action="cancel-edit-product">Cancel</button>` : ""}</div></form></section>`;
  }
  if (adminTab === "gallery") {
