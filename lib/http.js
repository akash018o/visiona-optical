...
// 4.2MB: comfortably fits a resized/compressed product photo (base64 adds
// ~33% overhead) while staying under Vercel's own ~4.5MB function body limit,
// so we return our own friendly error instead of a platform-level one.
...
export async function readBody(req) {
  ...
  for await (const chunk of req) {
    raw += chunk;
    if (raw.length > MAX_BODY_BYTES) throw new Error("That request is too large. If you're uploading a photo, please use one under 4MB.");
  }
  if (!raw) return {};
  try {
...
// Fails loudly rather than silently using a fallback secret — that fallback
// string would otherwise be sitting in this public repo, readable by anyone,
// which would let them forge a valid admin token if the real secret was ever
// missing from the environment.
function requiredTokenSecret() {
  const secret = process.env.TOKEN_SECRET;
  if (!secret) throw new Error("Server is not configured correctly. Please contact the site owner.");
  return secret;
}

export function sign(payload) {
  const tokenSecret = requiredTokenSecret();
...
export function verifyToken(header = "") {
  let tokenSecret;
  try {
    tokenSecret = requiredTokenSecret();
  } catch {
    return false;
  }
